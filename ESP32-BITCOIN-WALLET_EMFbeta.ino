...
// You need to a special resistor (pick a value between 1 megOhm - 3.3 megOhms) to connect ground and A2 (GPIO Pin 34)
// Then you will need to solder an antenna/piece of wire to the analog pin side of the resistor  
...
countdown = 1100;
  while (countdown >= (300))
  {delay(50); pixels.setPixelColor(0, pixels.Color(255,255,255));pixels.show();delay(50); pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();if (digitalRead(BUTTONA) == LOW){EMF();} if ( digitalRead(BUTTONB) == LOW){strobe();} if ( digitalRead(BUTTONC) == LOW){{pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();loop();}} --countdown;};
  while (countdown >= (0))
  {delay(50); pixels.setPixelColor(0, pixels.Color(255,0,0));pixels.show();pixels.show();delay(50); pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();if (digitalRead(BUTTONA) == LOW){EMF();}if ( digitalRead(BUTTONB) == LOW){strobe();} if ( digitalRead(BUTTONC) == LOW){{pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();loop();}} --countdown;};
