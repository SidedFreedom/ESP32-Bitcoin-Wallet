...
countdown = 1100;
  while (countdown >= (300))
  {delay(50); pixels.setPixelColor(0, pixels.Color(255,255,255));pixels.show();delay(50); pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();if ( digitalRead(BUTTONB) == LOW){strobe();} if ( digitalRead(BUTTONC) == LOW){{pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();loop();}} --countdown;};
  while (countdown >= (0))
  {delay(50); pixels.setPixelColor(0, pixels.Color(255,0,0));pixels.show();pixels.show();delay(50); pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();if ( digitalRead(BUTTONB) == LOW){strobe();} if ( digitalRead(BUTTONC) == LOW){{pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();loop();}} --countdown;};
...
//printer.feed(1);
printer.justify('L');
printer.println(F("Public Address 4: "));
printer.feed(1);
printer.printf(PBL3);
...
{Serial.println("SD did not initialize.");
  ...
  display.clearBuffer();
  display.setTextSize(1);
  testdrawtext("Check for SD Fat format, not Fat32. If formatted correctly, you may need to reseat the SD Card and try again.\n\n\nRevealing XPUB in 5 seconds.", COLOR1);
  display.display();
...
{ delay(100);
  ...
  display.clearBuffer();
  display.setTextSize(1);
//  testdrawtext("\nFlashlight | 3 Minutes\n\n\n-Press Top Left Button for More Time\n\n\n-Press Top Right Button to Exit", COLOR1);
  testdrawtext("\nFlashlight | 3 Minutes\n\n\n-Press Top Left Button for More Time\n\n\n-Press Top Middle Button for Strobe\n\n\n-Press Top Right Button to Exit", COLOR1);
  display.display();
  delay(500);

////placeholder for EMF detector "coming soon"  
//countdown = 1100;
//while (countdown >= 0)
//{
//delay(100);
//state = analogRead(Antenna);
...
countdown = 1100;
  while (countdown >= (300))
  {delay(100); pixels.setPixelColor(0, pixels.Color(255,255,255));pixels.show();if ( digitalRead(BUTTONA) == LOW){EMF();} if ( digitalRead(BUTTONB) == LOW){strobe();} if ( digitalRead(BUTTONC) == LOW){{pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();loop();}} --countdown;};
  while (countdown >= (0))
  {delay(100); pixels.setPixelColor(0, pixels.Color(255,0,0));pixels.show();if ( digitalRead(BUTTONA) == LOW){EMF();}if ( digitalRead(BUTTONB) == LOW){strobe();} if ( digitalRead(BUTTONC) == LOW){{pixels.setPixelColor(0, pixels.Color(0,0,0));pixels.show();loop();}} --countdown;};
